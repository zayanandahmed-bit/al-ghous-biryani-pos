-- AL GHOUS BIRYANI POS — Supabase database schema.
--
...
-- client is suffixed "_agb" so it lives completely separate from every
...
-- project's SQL Editor and click Run; it only creates new "_agb"
...
create table if not exists items_agb (
  id text primary key,
  name text not null,
...
alter table items_agb add column if not exists cost numeric default 0;
alter table items_agb add column if not exists expiry text default '';
alter table items_agb add column if not exists quick_pick boolean default false;

-- A master reference catalog — items_agb here are NOT live inventory (no stock
-- tracking, don't show up in Billing/Purchases) until explicitly copied into
-- the `items_agb` table via "Add to Inventory". Lets a store owner import a huge
-- premade product list without it looking like they stock everything in it.
create table if not exists item_catalog_agb (
  id text primary key,
  name text not null,
  barcode text default '',
  category text default '',
...
-- Safe to re-run — no-op if the column already exists.
alter table item_catalog_agb add column if not exists brand text default '';
alter table item_catalog_agb add column if not exists cost numeric default 0;

create table if not exists categories_agb (
  name text primary key
);

create table if not exists suppliers_agb (
  id text primary key,
  name text not null,
...
create table if not exists cashiers_agb (
  id text primary key,
  name text not null,
  pin text default ''
);

create table if not exists purchases_agb (
  id text primary key,
  date date,
  supplier_id text,
  supplier_name text,
...
-- General business expenses_agb (rent, utilities, salaries, misc) — "money out"
-- that ISN'T buying inventory stock (that's what `purchases_agb` already tracks).
-- Powers the Financials tab's Money In vs Money Out view.
create table if not exists expenses_agb (
  id text primary key,
...
-- Safe to re-run — no-op if the column already exists.
alter table expenses_agb add column if not exists period text default '';

-- Udhaar/credit accounts — a regular customer buys now, pays later.
-- `balance` is a running counter (like items_agb.stock), updated directly by the
-- app on every credit sale (+) and logged payment (-), not recomputed from
-- sales_agb history — keeps this consistent with how stock is already tracked.
create table if not exists customers_agb (
  id text primary key,
  name text not null,
  phone text default '',
  notes text default '',
...
-- Ledger of payments customers_agb make against their credit balance — kept
-- separate from `customers_agb` itself so there's a printable/auditable history.
create table if not exists customer_payments_agb (
  id text primary key,
  customer_id text,
...
create table if not exists sales_agb (
  id text primary key,
  receipt_no integer,
  date timestamptz,
  customer text,
...
create table if not exists refunds_agb (
  id text primary key,
  sale_id text,
  receipt_no integer,
  date timestamptz,
...
create table if not exists refund_lines_agb (
  refund_id text,
  item_id text,
  item_name text,
  qty numeric,
...
create table if not exists held_sales_agb (
  id text primary key,
  date timestamptz,
  customer text,
  discount numeric,
...
create table if not exists held_sales_cart_agb (
  held_id text,
  item_id text,
  qty numeric
);
...
create table if not exists active_shift_agb (
  id text primary key,
  cashier_id text,
  cashier_name text,
  start timestamptz,
...
create table if not exists settings_agb (
  key text primary key,
  value text
);

create table if not exists meta_agb (
  key text primary key,
  value text
);

-- Running totals, kept up to date by triggers below (not recomputed on
-- read). "All Time" stats/top-items_agb come from these instead of scanning the
-- full sales_agb/sale_lines_agb tables — the live scan approach that used to time
-- out once sales_agb history got into the tens of thousands. Any bounded range
...
create table if not exists item_sales_totals_agb (
  item_name text primary key,
  qty numeric default 0,
  revenue numeric default 0
);

create table if not exists sales_overall_totals_agb (
  key text primary key,
  count integer default 0,
  revenue numeric default 0
);
insert into sales_overall_totals_agb (key, count, revenue) values ('all', 0, 0)
on conflict (key) do nothing;

-- Staff on payroll — a plain roster (not a login/access-control table).
create table if not exists employees_agb (
  id text primary key,
  name text not null,
  phone text default '',
  role text default '',
  salary numeric default 0, -- monthly salary, for reference only, not auto-charged
  status text default 'active',
  join_date date,
  notes text default ''
);

-- Salary/payment ledger, grouped by `period` (month) in the UI — same
-- pattern as `expenses_agb.period`, so it reads the same way in Financials-
-- style views. Kept separate from `employees_agb` so there's a full history.
create table if not exists employee_payments_agb (
  id text primary key,
  employee_id text,
  employee_name text default '',
  period text default '', -- e.g. '2026-08' — which month this payment is FOR
  amount numeric default 0,
  date date, -- when it was actually paid
  notes text default ''
);
...
alter table items_agb enable row level security;
alter table item_catalog_agb enable row level security;
alter table categories_agb enable row level security;
alter table suppliers_agb enable row level security;
alter table cashiers_agb enable row level security;
alter table purchases_agb enable row level security;
alter table expenses_agb enable row level security;
alter table sales_agb enable row level security;
alter table sale_lines_agb enable row level security;
alter table refunds_agb enable row level security;
...
alter table meta_agb enable row level security;
alter table item_sales_totals_agb enable row level security;
alter table sales_overall_totals_agb enable row level security;
alter table customers_agb enable row level security;
alter table customer_payments_agb enable row level security;
alter table employees_agb enable row level security;
alter table employee_payments_agb enable row level security;

-- ============== triggers: keep running totals in sync ==============
-- Fire on every insert/update/delete to sales_agb/sale_lines_agb, regardless of
...
create or replace function trg_sale_lines_totals_agb()
returns trigger
language plpgsql
security definer
...
begin
  if TG_OP = 'INSERT' then
    insert into item_sales_totals_agb (item_name, qty, revenue)
    values (NEW.item_name, NEW.qty, NEW.subtotal)
    on conflict (item_name) do update set
      qty = item_sales_totals_agb.qty + NEW.qty,
      revenue = item_sales_totals_agb.revenue + NEW.subtotal;
    return NEW;
  elsif TG_OP = 'DELETE' then
    update item_sales_totals_agb set qty = qty - OLD.qty, revenue = revenue - OLD.subtotal
...
drop trigger if exists sale_lines_totals_trigger_agb on sale_lines_agb;
create trigger sale_lines_totals_trigger_agb
after insert or delete on sale_lines_agb
for each row execute function trg_sale_lines_totals_agb();

create or replace function trg_sales_totals_agb()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if TG_OP = 'INSERT' then
    update sales_overall_totals_agb set count = count + 1, revenue = revenue + NEW.grand where key = 'all';
    return NEW;
  elsif TG_OP = 'DELETE' then
    update sales_overall_totals_agb set count = count - 1, revenue = revenue - OLD.grand where key = 'all';
    return OLD;
  elsif TG_OP = 'UPDATE' then
    update sales_overall_totals_agb set revenue = revenue - OLD.grand + NEW.grand where key = 'all';
    return NEW;
  end if;
  return null;
end;
...
drop trigger if exists sales_totals_trigger_agb on sales_agb;
create trigger sales_totals_trigger_agb
after insert or update or delete on sales_agb
for each row execute function trg_sales_totals_agb();
...
insert into item_sales_totals_agb (item_name, qty, revenue)
select item_name, sum(qty), sum(subtotal) from sale_lines_agb group by item_name
on conflict (item_name) do update set qty = excluded.qty, revenue = excluded.revenue;

update sales_overall_totals_agb
set count = (select count(*) from sales_agb), revenue = (select coalesce(sum(grand), 0) from sales_agb)
where key = 'all';

-- ============== sync_push_agb: atomic full-replace write ==============

create or replace function sync_push_agb(payload jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if payload ? 'itemCatalog' then
    delete from item_catalog_agb where true;
    insert into item_catalog_agb (id, name, barcode, category, brand, price, cost, unit, low_stock)
  ...
  if payload ? 'customerPayments' then
    delete from customer_payments_agb where true;
    insert into customer_payments_agb (id, customer_id, customer_name, amount, date, notes)
  ...
  if payload ? 'employeePayments' then
    delete from employee_payments_agb where true;
    insert into employee_payments_agb (id, employee_id, employee_name, period, amount, date, notes)
    select x->>'id', x->>'employeeId', coalesce(x->>'employeeName',''), coalesce(x->>'period',''),
           coalesce((x->>'amount')::numeric,0), nullif(x->>'date','')::date, coalesce(x->>'notes','')
  ...
  if payload ? 'heldSales' then
    delete from held_sales_cart_agb where true;
  ...
  if payload ? 'receiptCounter' then
    insert into meta_agb (key, value) values ('receiptCounter', (payload->>'receiptCounter'))
    on conflict (key) do update set value = excluded.value;
  end if;
end;
$$;

-- ============== sync_pull_agb: read everything EXCEPT sales_agb/refunds_agb ==============
-- Sales and refunds_agb are deliberately left out here — that history can grow
-- without bound, and building/sending all of it on every app load is what
-- broke this at high volume. Sales/refunds_agb are fetched separately, scoped to
-- whatever date range the user has selected, by sync_pull_sales_range_agb below.

create or replace function sync_pull_agb()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  select jsonb_build_object(
    ...
    'employeePayments', coalesce((select jsonb_agg(jsonb_build_object(
      'id', id, 'employeeId', employee_id, 'employeeName', employee_name, 'period', period,
      'amount', amount, 'date', date, 'notes', notes
    )) from employee_payments_agb), '[]'::jsonb),
...
-- ============== indexes for range-scoped sales_agb/refunds_agb lookups ==============

create index if not exists idx_sales_date_agb on sales_agb(date);
create index if not exists idx_refunds_date_agb on refunds_agb(date);
create index if not exists idx_sale_lines_sale_id_agb on sale_lines_agb(sale_id);
create index if not exists idx_refund_lines_refund_id_agb on refund_lines_agb(refund_id);

-- ============== sync_pull_sales_range_agb: one page of sales_agb/refunds_agb for a date range ==============
...
--   - sales_agb/refunds_agb: one page (page_size rows, newest-first) — pass
...
--     LATERAL join so only the selected page's line items_agb get aggregated,
--     never the whole sale_lines_agb table, however large it's grown.
...
--     once sales_agb history got into the tens of thousands.
-- Pass null for range_start/range_end for an unbounded (All Time) range.

create or replace function sync_pull_sales_range_agb(
...
begin
  if is_all_time then
    select count, revenue into v_total_count, v_total_revenue from sales_overall_totals_agb where key = 'all';
    select coalesce(jsonb_agg(jsonb_build_object('name', item_name, 'qty', qty, 'revenue', revenue) order by qty desc), '[]'::jsonb)
    into v_top_items
    from (select * from item_sales_totals_agb order by qty desc limit 20) t;
  else
    select count(*) into v_total_count from sales_agb
    where (range_start is null or date >= range_start) and (range_end is null or date <= range_end);

    select coalesce(sum(grand), 0) into v_total_revenue from sales_agb
    where (range_start is null or date >= range_start) and (range_end is null or date <= range_end);
  ...
  -- pre-aggregated the ENTIRE sale_lines_agb table before the join happened, so
  ...
  left join lateral (
    ...
    from sale_lines_agb where sale_id = t.id
  ) sl on true;
    ...
    'refunds', coalesce((
      select jsonb_agg(x order by x->>'date' desc) from (
        ...
        left join lateral (
          ...
          from refund_lines_agb where refund_id = r.id
        ) rl on true
...
-- ============== sync_append_sale_agb: instant single-sale insert ==============
-- Used right after checkout, on every device, so a sale reaches the database
-- immediately without resending the entire sales_agb history each time. Safe to
-- retry (e.g. after a dropped connection) since it upserts on the sale's id.

create or replace function sync_append_sale_agb(sale jsonb, receipt_counter integer default null)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from sale_lines_agb where sale_id = sale->>'id';
  insert into sale_lines_agb (sale_id, item_id, item_name, barcode, price, qty, unit, subtotal)
  ...
  if receipt_counter is not null then
    insert into meta_agb (key, value) values ('receiptCounter', receipt_counter::text)
    on conflict (key) do update set value = excluded.value;
  end if;
end;
$$;

-- ============== sync_append_sales_batch_agb: bulk append, no wipe ==============
-- Set-based insert for many sales_agb at once (e.g. bulk-loading historical data)
-- without ever deleting the existing table first, unlike sync_push_agb. Callers
-- should chunk large loads (a few thousand sales_agb per call) to stay under
...
-- ============== sync_append_refund_agb: instant single-refund insert ==============

create or replace function sync_append_refund_agb(refund jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from refund_lines_agb where refund_id = refund->>'id';
  insert into refund_lines_agb (refund_id, item_id, item_name, qty, price, refund_amount)
...
-- ============== sync_clear_sales_agb: wipes sales_agb/sale_lines_agb only ==============
-- Used by "Clear all sales_agb history" — a deliberate one-off admin action,
-- not something that happens per checkout.

create or replace function sync_clear_sales_agb()
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function sync_replace_item_catalog_agb("itemCatalog" jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function sync_replace_customer_payments_agb("customerPayments" jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function sync_replace_employee_payments_agb("employeePayments" jsonb)
returns void
language plpgsql
security definer
set search_path = public
...
grant execute on function sync_push_agb(jsonb) to anon;
grant execute on function sync_pull_sales_range_agb(timestamptz, timestamptz, integer, timestamptz, text) to anon;
...
drop function if exists sync_pull_sales_range_agb(timestamptz, timestamptz, integer);
grant execute on function sync_pull_agb() to anon;
grant execute on function sync_append_sale_agb(jsonb, integer) to anon;
...
grant execute on function sync_replace_suppliers_agb(jsonb) to anon;
grant execute on function sync_replace_cashiers_agb(jsonb) to anon;
grant execute on function sync_replace_purchases_agb(jsonb) to anon;
grant execute on function sync_replace_expenses_agb(jsonb) to anon;
grant execute on function sync_replace_customers_agb(jsonb) to anon;
grant execute on function sync_replace_customer_payments_agb(jsonb) to anon;
...
-- ('pos-al-ghous-biryani' or 'client-manager') so their credentials are
...
insert into app_logins (app_name, username, password_hash)
select 'pos-al-ghous-biryani', 'admin', extensions.crypt('changeme123', extensions.gen_salt('bf'))
where not exists (select 1 from app_logins where app_name = 'pos-al-ghous-biryani');
